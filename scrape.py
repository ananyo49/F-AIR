...
def ScrapeObjects(objs, deep_copy=False):
    ...
    debug = [obj._scrape_data(driver)
             for obj in tqdm(objs, desc="Scraping Objects")]
...
class _Scrape:
    ...
    def __add__(self, other):
        ...
        if obj_type == 'one-way':
            ...
            assert datetime.strptime(self.date[1], date_format) < datetime.strptime(
                other.date[0], date_format), "Dates are not in order. Make sure to provide them in increasing order in YYYY-MM-DD format."
    ...
    def _set_properties(self, *args):
        ...
        # round-trip
            ...
            assert datetime.strptime(args[2], date_format) < datetime.strptime(
                args[3], date_format), "Dates are not in order. Make sure to provide them in increasing order in YYYY-MM-DD format."

            self._origin, self._dest, self._date = [
                args[0], args[1]], [args[1], args[0]], args[2:]
        ...
        # chain-trip, chain is component of 3s, check that last one is an actual date to not confuse w perfect
            ...
            for i in range(0, len(args), 3):
                ...
                assert len(args[i + 1]) == 3 and type(args[i+1]
                                                      ) == str, "Issue with arg {}, see docs".format(i+1)
                assert len(args[i + 2]) == 10 and type(args[i + 2]
                                                       ) == str, "Issue with arg {}, see docs".format(i+2)

                if i > 0:
                    assert datetime.strptime(self._date[-1], date_format) < datetime.strptime(
                        args[i + 2], date_format), "Dates are not in order ({d1} > {d2}). Make sure to provide them in increasing order in YYYY-MM-DD format.".format(d1=self._date[-1], d2=args[i+2])
        ...
        # perfect-chain
            ...
            for i in range(2, len(args)-1, 2):
                assert len(args[i]) == 3 and type(
                    args[i]) == str, "Issue with arg {}, see docs".format(i)
                assert len(args[i + 1]) == 10 and type(args[i + 1]
                                                       ) == str, "Issue with arg {}, see docs".format(i+1)
                assert datetime.strptime(self._date[-1], date_format) < datetime.strptime(
                    args[i + 1], date_format), "Dates are not in order ({d1} > {d2}). Make sure to provide them in increasing order in YYYY-MM-DD format.".format(d1=self._date[-1], d2=args[i+1])

                self._origin += [args[i]]
                self._dest += [args[i]]
                self._date += [args[i+1]]

            assert len(args[-1]) == 3 and type(args[-1]
                                               ) == str, "Issue with last arg, see docs"
            self._dest += [args[-1]]
    ...
    def _scrape_data(self, driver):
        results = [self._get_results(url, self._date[i], driver)
    ...
    def _clean_results(result, date):
        ...
        flights = [Flight(date, res3[matches[i]:matches[i+1]])
